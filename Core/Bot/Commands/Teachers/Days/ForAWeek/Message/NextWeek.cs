...
namespace Core.Bot.Commands.Teachers.Days.ForAWeek.Message {
    ...
    internal class TeachersNextWeek : IMessageCommand {
        ...
        public async Task Execute(ScheduleDbContext dbContext, ChatId chatId, int messageId, TelegramUser user, string args) {
            ...
            foreach((string, DateOnly) item in Scheduler.GetTeacherWorkScheduleByWeak(dbContext, CultureInfo.CurrentCulture.Calendar.GetWeekOfYear(DateTime.Now, CalendarWeekRule.FirstFourDayWeek, DayOfWeek.Monday), user.TelegramUserTmp.TmpData!))
