...
namespace Core.Bot.Commands.Student.Custom.Message {
    internal class CustomEditLectureHall : IMessageCommand {
        ...
        public List<Mode> Modes => new() { Mode.CustomEditLectureHall };
        ...
        public async Task Execute(ScheduleDbContext dbContext, ChatId chatId, int messageId, TelegramUser user, string args) {
            if(!string.IsNullOrWhiteSpace(user.TempData)) {
                ...
                await BotClient.SendTextMessageAsync(chatId: chatId, text: "Аудитория успешно изменена.", replyMarkup: Statics.MainKeyboardMarkup);
