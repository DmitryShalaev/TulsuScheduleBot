...
namespace ScheduleBot.Bot {
    ...
    public partial class TelegramBot {
        ...
        private async Task UpdatedDisciplinesAsync(ScheduleDbContext dbContext, List<(string Group, DateOnly Date)> values) {
            ...
            foreach(var (Group, Date) in values) {
                ...
                var days = (DateTime.Parse(Date.ToString()) - DateTime.Now.Date).TotalDays;

                foreach(var user in telegramUsers.Where(i => i.ScheduleProfile.Group == Group && days <= i.Notifications.Days)) {
                    if(!user.Flag) {
