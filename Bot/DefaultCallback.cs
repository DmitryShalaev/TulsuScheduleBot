            ...
            if(DateOnly.TryParse(message.Text?.Split('-')[0].Trim()[2..] ?? "", out DateOnly date)) {
                ...
                switch(data) {
                    case Constants.IK_Edit.callback:
                        if(IsAdmin)
                            await botClient.EditMessageTextAsync(chatId: message.Chat, messageId: message.MessageId, text: scheduler.GetScheduleByDate(date, user.ScheduleProfile, true), replyMarkup: GetEditAdminInlineKeyboardButton(date, user.ScheduleProfile));
                        else
                            await botClient.EditMessageTextAsync(chatId: message.Chat, messageId: message.MessageId, text: scheduler.GetScheduleByDate(date, user.ScheduleProfile), replyMarkup: inlineKeyboardMarkup);
                        break;
                    ...
                    default:
                        if(IsAdmin) {
                            ...
                            await botClient.EditMessageReplyMarkupAsync(chatId: message.Chat, messageId: message.MessageId, replyMarkup: GetEditAdminInlineKeyboardButton(date, user.ScheduleProfile));
                        } else
                            await botClient.EditMessageTextAsync(chatId: message.Chat, messageId: message.MessageId, text: scheduler.GetScheduleByDate(date, user.ScheduleProfile), replyMarkup: inlineKeyboardMarkup);
